...
namespace EasyRun.Tye
{
    public class TyeManager
    {
        ...
        public string BuildTyeManifest(DTE2 dte, ProfileModel profile, Guid instanceId)
        {
            ...
            string existingTyeFile = Path.Combine(solutionPath, "tye.yaml");
            if (System.IO.File.Exists(existingTyeFile))
                return existingTyeFile;
            else
                try
                {
                    // Service Name
        ...
        private async System.Threading.Tasks.Task<bool> QueryTyeServicesAsync(DTE2 dte, long number, ProfileModel profile)
        {
            ...
            if (tyeServices != null && tyeServices.Any())
            {
                var servicesFromProfile = profile.FilteredServices.Select(fs => fs.TyeName).ToList();

                var ourTyeRunnedServices = tyeServices.Where(service => servicesFromProfile.Contains(service.Description.Name)).ToList();

                var pidDic = GetPidList(ourTyeRunnedServices);

                if (servicesFromProfile.Count == pidDic.Count)
                {
                    UnsubscribeTyeQuery();
        ...
        private Dictionary<int, ReplicaItem> GetPidList(List<TyeServiceDTO> tyeServices, string name = null)
        {
            ...
            foreach (var service in tyeServices)
            {
                if (name is null || service.Description.Name == name)
                {
                    ...
                    {
                        if (replica.Pid.HasValue)
                            pidList.Add(replica.Pid.Value, new ReplicaItem { ServiceName = service.Description?.Name, ReplicaName = replica.Name });
                    }
                }
            }
